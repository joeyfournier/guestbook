...
/**
 * NOTE: this is pretty much a direct copy of openid4java's example ConsumerServlet with
 * modifications to check for valid subscriptions/users that match 
 * successfully verified OpenId users.
 * 
 */
public class ConsumerServlet extends javax.servlet.http.HttpServlet {
	...
	private void processReturn(HttpServletRequest req, HttpServletResponse resp)
		...
		if (identifier == null) {
			...
			String theIdentifier = identifier.getIdentifier();
			req.setAttribute("identifier", identifier.getIdentifier());


			
			// check if our subscriptions contain this user
			

	      List<Subscription> subscriptions = ObjectifyService.ofy()
	          .load()
	          .type(Subscription.class) // We want only Greeting
	          .list();
	        boolean userFound = false;
	        String message = "Invalid+User";
		    if (subscriptions.isEmpty()) {
				// no subscription, redirect to purchase page
				this.getServletContext().getRequestDispatcher("/purchase.jsp?message=No+Subscription")
				.forward(req, resp);
		    } else {
		    	
		        //  Check for our user in all our subscriptions (either the creator or 
		        // TODO: We should be able to break this down to correct subscription based on OpenId identifer? or does it matter is he is a valid user for any sub let him in?
		        for (Subscription sub : subscriptions) {
		        	if ((sub.theUsers!=null && sub.theUsers.containsKey(theIdentifier)) || (sub.creator != null && theIdentifier.equals(sub.creator.getOpenId())))
		     		{
		        	    // subscription must be valid	 
		        		if (HandleAppDirectServlet.STATUS_ACTIVE.equalsIgnoreCase(sub.getStatus()))
		        		{
			        		userFound = true;
			...
			{   
				...
				mySession.setAttribute("appDirectAccessValidated", "TRUE");
				this.getServletContext().getRequestDispatcher("/guestbook.jsp")
				.forward(req, resp);
				
			}
			// no user with valid sub found, insert no supscription message here
			// SORRY THIS APPLICATION REQUIRES a valid subscription and to single sign on with AppDirect's marketplace. 
			this.getServletContext().getRequestDispatcher("/purchase.jsp?message="+message)
			.forward(req, resp);
		}
	}
